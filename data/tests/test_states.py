...
class TestStates:
    ...
    def sample_state(self):
        """Sample state data."""
        return {
            S.STATE_NAME: 'New York', S.STATE_CODE: 'NY',
            S.COUNTRY_CODE: 'US', S.CAPITAL: 'Albany',
            S.POPULATION: 20201249, S.AREA_KM2: 141297
        }
    ...
    def test_add_state_invalid_code_pattern_raises(self, sample_state):
        ...
        with patch('data.states.get_state_by_code', return_value=None), \
             patch('data.db_connect.create') as mock_create:
            mock_create.side_effect = Exception("Document failed validation: state_code pattern")
    ...
    def test_add_state_negative_population_raises(self, sample_state):
        ...
        with patch('data.states.get_state_by_code', return_value=None), \
             patch('data.db_connect.create') as mock_create:
            mock_create.side_effect = Exception("validation: population must be >= 0")
            with pytest.raises(Exception, match="population must be >= 0"):
                S.add_state(invalid_state)
    ...
    def test_update_state_strips_code_from_update(self, sample_state, mock_modified_result):
    ...
    def test_update_state_with_patch_object(self, sample_state, mock_modified_result):
        ...
        with patch.object(S, 'get_state_by_code', return_value=sample_state), \
             patch('data.db_connect.update', return_value=mock_modified_result):
            assert S.update_state('NY', {S.CAPITAL: 'New Albany'}) is True

    def test_update_state_transient_failure_side_effect(self, sample_state, mock_modified_result):
